...
The latest PressReader SDK can be downloaded from [release page](https://github.com/pressreader/pr-android-full-sdk/releases). Make sure to request access to GitHub repository. For receiving the SDK release notifications, subscribe to Watch → Custom → Releases event.
...
Sample apps is provided with SDK package in folders `app_sdk_full` (PressReader flow).

# Local Server

To use the `app_sdk_full` sample with a local server, define the local server URL in resources:
